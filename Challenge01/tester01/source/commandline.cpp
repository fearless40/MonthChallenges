...
const Options &get_program_options() { return GlobalOptions; }

std::optional<std::filesystem::path>
ensure_file_exists(std::string_view filename) {
...
std::optional<Options> parse(int argc, char *argv[]) {
  ...
  auto commandRun =
      (clipp::command("run").set(opt.mode, RunMode::Run),
       value("test file", testFile) %
       ...
       required("-p", "--program") &
           value("program to test", testProgram) %
               "Specifify the program to test. It should follow the Challenge "
               "descriptions.");

  auto commandGenerate =
      (clipp::command("generate").set(opt.mode, RunMode::Generate),
       value("test file", testFile) % "The test file that desribes the tests "
                                      "and location of the test files.",
       (option("--NoErrors").set(opt.tests, TestModes::NoErrors) %
            "Generate only test files that are valid. No errors in the test "
            "files." |
        option("--all").set(opt.tests, TestModes::All) %
            "Generate all test files this is the default."),
       option("--huge").set(opt.huge) % "Generate a huge test file.");

  auto cli = (commandRun | commandGenerate |
                  command("help").set(opt.mode, RunMode::Help),
              option("-v", "--version")
  ...
  if (!result) {
    std::cout << "Usuage: \n" << clipp::usage_lines(cli, "apptest") << '\n';
    return {};
  }

  if (opt.mode == RunMode::Help) {
    std::cout << clipp::make_man_page(cli, "apptest") << '\n';
    return {};
  }
  ...
  case RunMode::Run: {
    ...
    if (!fileTest) {
      std::cout << "The test file is not found: " << testFile << '\n';
      return {};
    }
